...
| **[agentfield-control-plane](https://github.com/phuongddx/agentfield-control-plane)**<br>`React` `TypeScript` `SSE` | Observability surface for AI agent runs — per-run **DAG viewer with step traces**, side-by-side run comparison, a playground for exercising reasoners, **per-model circuit-breaker states** (open / half-open / closed), and signed-provenance verification that surfaces tampering. |
